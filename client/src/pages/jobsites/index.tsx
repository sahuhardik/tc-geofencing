    ...
    <>
      <Card className="flex flex-col xl:flex-row items-center mb-8">
        <div className="md:w-1/3 mb-4 xl:mb-0">
          <h1 className="text-xl font-semibold text-heading">{t('common:text-jobsites')}</h1>
        </div>

        <div className="w-full xl:w-2/3 flex flex-col md:flex-row space-y-4 md:space-y-0 items-center ms-auto">
          <Search onSearch={handleSearch} />

          <LinkButton href={`${ROUTES.JOBSITES}/create`} className="h-12 md:ms-6 w-full md:w-auto">
            <span>+ {t('form:button-label-add-jobsite')}</span>
          </LinkButton>
        </div>
      </Card>

      <JobSiteList jobsites={data?.jobsites} onPagination={handlePagination} onOrder={setOrder} onSort={setColumn} />
    </>
  );
}
