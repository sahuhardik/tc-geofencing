...
const Attribute: React.FC<AttributeProps> = ({ value, active, className, color, ...props }) => {
  const classes = cn(
    {
  ...
  return (
    <div className={classes} {...props}>
      {className === 'color' ? (
        <span className="w-full h-full rounded-full border border-border-200" style={{ backgroundColor: color }} />
      ) : (
        value
      )}
    </div>
