  ...
  const jobsiteMapsData: IJobSiteMap[] = jobSites.map(
    (jobSite, i) =>
      ({
        ...
        members: jobSite.jobSiteUsers?.map(
          (jobsiteUser) =>
            ({
  ...
  const buildMembersiteCard = (memberCardData: {
    ...
    <div class=${styles.jobsiteMapCardHeader} >
      ...
      <div class=${styles.jobsiteMapHeaderTitle} >
        ...
        <span class=${styles.jobsiteCardSubHeading} >${memberCardData.jobsiteName} - ${memberCardData.jobsiteAddress}</span>
      </div>
    </div>
    ...
    <div class=${styles.jobsiteCardActiveUsers} >
      ...
      ${jobsiteMapData.members
        ...
        ${buildMemberIcon(
          ...
          <span style="height: 10px; width: 10px; border-radius: 5px; background-color: ${ member.isActive ? '#008000' : '#b30000'}; margin-left: 2px; line-height: 12px;" ></span>
        </div>
      `
        )
        .join('')}
