...
import Svgs from "@assets/placeholders/JobsiteMapIcon.svg";
...
const jobSites: IJobSiteMap[] = [
  {
    name: 'My jobsite name',
    radius: 10,
    markerColor: '#A12687',//'#5B4D99',
    position: {
      lat: 22.7094215,
      lng: 75.87266,
    },
...
function highlight(markerView: any) {
  markerView.content.classList.add('highlight');
  markerView.element.style.zIndex = 1;
}

function unhighlight(markerView: any) {
...
export const JobsiteMapWidget = ({ height, zoom = 12 }: { height: string; zoom?: number }) => {
  return (
      <div style={{ display: 'flex', height, }}>
        <Wrapper apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAP_API_KEY!} version="beta" mapIds={['theuniquemapid']} >
          <Map center={jobSites[0].position} zoom={zoom} style={{ flexGrow: '1', height: '100%' }} mapId={'theuniquemapid'} >
            {
              jobSites.map((jobSite) => <JobSiteMap markerColor={jobSite.markerColor} position={jobSite.position} radius={jobSite.radius} members={jobSite.members} name={jobSite.name} />)
            }
          </Map>
        </Wrapper>
      </div>
...
export const MemberMarker = (props: IMemberMarker) => {
  const buildIcon = (imgSrc?: string) => {
    if (!imgSrc) {
      return '';
    }
    return `<img
        src="${imgSrc}"
        style="height: 16px; width 16px; border-radius: 7px; margin: 0px;"
      />`;
    }
  return (
    <AdvanceMarker
      ...
      markerIconHtml = {buildIcon(props.avatar)}
...
const JobSiteMap = (props: IJobSiteMap) => {
  ...
  return <>
      ...
      {props.members.map((member) => (
             <MemberMarker {...member} map={props.map} mapRef={props.mapRef} />
      ) )}
      <Circle map={props.map} center={props.position} fillColor={props.markerColor} strokeColor={props.markerColor} radius={props.radius} />
      </>
};
